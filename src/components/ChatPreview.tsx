...
function ChatPreview(props: {
  ...
  useEffect(() => {
    const fetchPreview = async (): Promise<void> => {
      ...
      if (chatRes) {
        chatRes.participants = chatRes.participants.filter(
          (id: string) => id !== props.currentUser
        );
        let user: IUser = (await fetchInfoAboutUser(
          chatRes.participants[0]
        )) as IUser;
        if (user) {
          setData(user);
        }
  ...
  return (
    <Box
      onClick={() => props.fetchChat(props.chat)}
      sx={{
        display: "flex",
        alignItems: "center",
