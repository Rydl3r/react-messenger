...
function Sidebar(props: { user: any; setOpenedChat: (value: IChat) => void }) {
  ...
  const createChat = async () => {
    const neededIndex: number = allUsersInfo?.findIndex(
      (elem: any) => elem["email"] === emailQuery
    ) as number;
    if (
      currentUserInfo &&
      currentUserInfo.friends &&
      currentUserInfo.friends.includes(emailQuery)
    ) {
      ...
      const docRef = await addDoc(collection(firestore, "chats"), {
        ...
        content: [{ from: "both", message: "The chat has started" }],
      });
      ...
      await updateDoc(
        doc(firestore, "users", props.user.uid),
        newCurrentUserInfo
      );
  ...
  return (
    ...
    >
      ...
      <Button
        sx={{ my: 2, display: "flex", mx: "auto" }}
        variant="contained"
        onClick={() => setModalOpen(true)}
      >
        <AddCircleIcon />
        <Typography sx={{ pl: 1 }} className="small_screen">
          Start new chat
        </Typography>
      </Button>
      ...
      {currentUserInfo &&
      currentUserInfo.startedChats &&
      currentUserInfo.startedChats.length > 0
        ? currentUserInfo.startedChats.map((chat: string) => (
            <ChatPreview
              key={chat}
              currentUser={props.user.uid}
              chat={chat}
              fetchChat={fetchChat}
            />
          ))
